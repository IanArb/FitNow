...
public class WalkLeadersStepsAdapter extends FirebaseRecyclerAdapter<RunWalkModel, WalkLeadersViewHolder> {

  Context context;

  public WalkLeadersStepsAdapter(Class<RunWalkModel> modelClass, int modelLayout, Class<WalkLeadersViewHolder> viewHolderClass, Query ref, Context context) {
  ...
  private ImageView getViews(WalkLeadersViewHolder viewHolder, RunWalkModel model, int position) {
    ...
    String formatSteps = StringUtils.formatInt(model.getSteps());
    tvScore.setText(formatSteps);
